...
async function fetchOrderData(tradeNo, isPolling = false) {
    try {
        ...
        console.error(err);
        if (!isPolling) showError(t('network_error'));
    }
}
...
function renderOrder(data, msg, tradeNo) {
    ...
    if (data.card_number || data.banktransf_account) {
        ...
        const remarkRow = document.getElementById('remark-row');
        if (remarkRow && data.remark) {
            remarkRow.style.display = 'flex';
            document.getElementById('order-remark').innerText = data.remark;
        }
        ...
        if (qrPlaceholder.innerHTML.indexOf('canvas') === -1) {
            ...
            if (!qrTextContainer) {
                ...
                qrTextContainer.innerHTML = `
                    <button class="copy-btn" id="save-qr-btn" style="margin-top:8px; width:100%; padding:8px; font-size:14px; background: #4F46E5; color: white;" data-i18n="btn_save_qr">Save QR Code</button>
                `;
